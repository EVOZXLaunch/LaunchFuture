...
function updateNetworkButton() {
    ...
    if (networkButton) networkButton.textContent = net.symbol || net.name;
}
...
function bindEvents() {
    ...
    document.getElementById("prevButton")?.addEventListener("click", () => {
        const step = getWizardData ? undefined : undefined;
        showStep(getCurrentStepNum() - 1);
    });
...
// Track current step locally
let _currentStep = 1;
function getCurrentStepNum() { return _currentStep; }

// Override showStep to track step
...
function updatePreview() {
    ...
    if (previewFeatures) previewFeatures.textContent = `${enabledFeatures} Enabled`;
}
